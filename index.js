...
const { horizontal, vertical } = data.reduce((state, action) => {
    ...
    const dHorizontal = (direction === DIRECTIONS.EAST ? 1 : (direction === DIRECTIONS.WEST ? -1 : 0)) * moves;  
    const dVeritcal = (direction === DIRECTIONS.NORTH ? 1 : (direction === DIRECTIONS.SOUTH ? -1 : 0)) * moves;
